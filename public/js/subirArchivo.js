document.addEventListener('DOMContentLoaded', function () {
    ...
    // Verificamos si los elementos existen para evitar errores en consola
    if (fileInput && btnAnalyze) {
        fileInput.addEventListener('change', function () {
            if (this.files && this.files.length > 0) {
                ...
                // 2. Estilo visual de éxito (Borde esmeralda)
                labelFile.classList.remove('border-blue-200', 'text-blue-600');
                labelFile.classList.add('border-emerald-400', 'bg-emerald-50', 'text-emerald-600');

                // 3. Habilitar botón y cambiar a color azul
                btnAnalyze.disabled = false;
                btnAnalyze.classList.remove('bg-slate-300', 'cursor-not-allowed');
                btnAnalyze.classList.add('bg-blue-600', 'hover:bg-blue-700', 'shadow-lg', 'cursor-pointer');
            }
        });
    }

    // Efecto de carga al enviar
    if (uploadForm && btnAnalyze) {
        uploadForm.addEventListener('submit', function() {
            // Cambiamos el contenido del botón para dar feedback al usuario
